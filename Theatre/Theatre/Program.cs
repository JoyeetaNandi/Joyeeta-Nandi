...
namespace Theatre
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            eglinton.AddShow(new Show(toys4, Day.Sat, 6.40, new Time(14, 10,0))); //check

            eglinton.AddShow(new Show(godzilla, Day.Mon, 6.89, new Time(13, 55, 0)));
            eglinton.AddShow(new Show(avengers, Day.Sat, 12.25, new Time(21, 5, 0)));
            eglinton.AddShow(new Show(godzilla, Day.Sun, 6.89, new Time(14, 0,0))); //check
            eglinton.AddShow(new Show(toys4, Day.Sat, 6.40, new Time(14, 10,0))); //check
            eglinton.AddShow(new Show(avengers, Day.Sat, 12.25, new Time(21, 5, 0)));
            ...
            eglinton.AddShow(new Show(godzilla, Day.Wed, 8.50, new Time(22, 5, 0)));//check
            eglinton.AddShow(new Show(avengers, Day.Tue, 10.75, new Time(20, 30, 0)));//check
            eglinton.AddShow(new Show(godzilla, Day.Thu, 8.50, new Time(20, 15, 0)));//check
            eglinton.AddShow(new Show(avengers, Day.Wed, 10.75, new Time(20, 30, 0)));//check
            eglinton.AddShow(new Show(godzilla, Day.Fri, 8.50, new Time(18, 25, 0)));//check
            eglinton.AddShow(new Show(avengers, Day.Sun, 10.75, new Time(14, 15, 0)));//check
            //Console.WriteLine("27 movies");
            eglinton.PrintShows();                              //displays 27 objects
            //Console.WriteLine("8 movies");
            eglinton.PrintShows(Day.Sun);                       //displays 8 objects
            //Console.WriteLine("19 movies");
